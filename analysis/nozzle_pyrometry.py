...
CAMERA = PyroCamera()            # 5 um pitch, F/4, 5 e- read, 20 ke- well

# Candidate pairs [centre_short, width_short, centre_long, width_long] (nm)
CANDIDATE_PAIRS = {
    "450/650 (user's idea)": (450, 30, 650, 30),
...
def make_pyro(l1_nm, w1_nm, l2_nm, w2_nm):
...
def frame_exposures(pyro):
    ...
    return (
        exposure_for_well_fill(T_PRIMARY_HI, pyro.filter_short, CAMERA, WELL_FILL),
        exposure_for_well_fill(T_PRIMARY_HI, pyro.filter_long, CAMERA, WELL_FILL),
    )
...
def fig_spectra():
    ...
    # Silicon response window
    qe = silicon_qe(lam)
    ax2 = ax.twinx()
    ax2.fill_between(lam * 1e9, 0, qe, color="gray", alpha=0.15, lw=0)
    ax2.set_ylabel("Silicon QE (shaded)", color="gray")
    ax2.set_ylim(0, 1.6)
    ...
    ax.set_title("Nozzle blackbody spectra on the silicon window; "
...
def fig_sigma_vs_temperature():
    ...
    fig, ax = plt.subplots(figsize=(9, 5.5))
    colors = {"450/650 (user's idea)": "#6c71c4",
...
def fig_channel_dynamics():
    ...
    ax.text(700, CAMERA.well_capacity * 1.3, "full well (20 ke-)", fontsize=8)
    ax.axhline(WELL_FILL * CAMERA.well_capacity, color="black", ls="--", lw=0.8)
    ax.axhline(CAMERA.read_noise, color="gray", ls=":")
    ax.text(700, CAMERA.read_noise * 1.4, "read noise (5 e-)", fontsize=8,
            color="gray")
...
def main():
    ...
    add("*Machine-generated by `analysis/nozzle_pyrometry.py` -- do not edit "
        "by hand.*\n")
    add(f"Camera model: {CAMERA.pixel_pitch * 1e6:.0f} um pixels, "
        f"F/{CAMERA.f_number:g}, QE = typical silicon CMOS, read noise "
        f"{CAMERA.read_noise:.0f} e-, well {CAMERA.well_capacity / 1e3:.0f} ke-, "
        f"optics+window transmittance {CAMERA.optics_transmittance}. Exposures "
        f"anti-saturated to {WELL_FILL * 100:.0f}% well at 3000 C.\n")
